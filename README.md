...
```json
{
  ...
  "message": "导入失败：共重启持久化验证 | 3 | 模拟重启后数据和配置保持一致、配置不跳回旧版本、历史不丢失条无效记录"
}
```
...
| 重启持久化验证 | 3 | 模拟重启后数据和配置保持一致、配置不跳回旧版本、历史不丢失|重启持久化验证 | 3 | 模拟重启后数据和配置保持一致、配置不跳回旧版本、历史不丢失|重启持久化验证 | 3 | 模拟重启后数据和配置保持一致、配置不跳回旧版本、历史不丢失 |
...
| 备注接口 - 正确方法与路径 |重启持久化验证 | 3 | 模拟重启后数据和配置保持一致、配置不跳回旧版本、历史不丢失| PATCH 添加备注成功，GET 详情能查到备注 |
